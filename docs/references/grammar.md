...
**What it does *not* do:** It does **not** set the Go `package` line in emitted files, and the **`.fb` file name is not used** for any of this. The compiler reads the path only to load the file. The package name comes from the code generator’s `-p` flag, or—if `-p` is omitted—from the **last path segment of the output directory** (`-o`); see [cmd/main.go](../../cmd/main.go). A generated directory like `package mylang_gen` can therefore sit beside `grammar MyLanguage;` in one project: the two names are unrelated unless you align them on purpose.
